...
<p align="center">
   ...
   <img src="images/accuracy_Ivam.png" alt=""  height=350/> 
</p>

<p align="right">
  <em><small>Figure 6-7</small></em>
</p>

The confusion matrix shows that the model has a good ability to distinguish between the most common classes. Particularly, the graph displays 100% accuracy for classes 358.0 and 317.0.
Most of the exemption codes exhibit very high accuracy, except for classes 336.0, 354.0, 375.0, OTHER, 300.0, and 346.0, which have accuracies of 88%, 87%, 83%, 80%, 75%, and 60% respectively. Although it is not possible to definitively say that a low number of occurrences leads to reduced accuracy, since the best-predicted class, 358.0 (which has 262 occurrences), does not have a high number of occurrences compared to others such as 324.0 (which has 5218 occurrences), it is observable that all the classes with lower accuracy do have fewer occurrences.
...
In conclusion, the dataset was cleaned by handling NaN values, ensuring the integrity and reliability of our data, moving on developing various machine learning algorithms in order to evaluate how these models effectively predicted IvaM exemption codes. Through accuracy, F1 Score and the comparison with the benchmark it was assessed the performance of each model and selected the most suitable one for predicting exemption codes. Finally, the results were interpreted to extract valuable insights and inform decision-making processes.

While the performance of the model is extremely satisfying, it is evident its limitation in predicting exemption codes categorized under the class 'OTHER'. This issue could potentially be addressed by utilizing a more balanced dataset. By ensuring that the dataset includes a sufficient representation of instances from each exemption code category, including those within the 'OTHER' class, it can provide the model with a more comprehensive understanding of the underlying patterns and relationships. This approach may enable the model to better capture the nuances associated with predicting exemption codes across all classes, thereby enhancing its overall predictive accuracy and effectiveness.

Overall, the performance of our model is appreciably adequate. This satisfaction stems from its consistent adherence to established quality benchmarks and its ongoing improvement towards achieving optimal predictive capabilities. This reliability instils confidence in the model's ability to make accurate predictions, contributing to overall confidence in its effectiveness for the task at hand.
